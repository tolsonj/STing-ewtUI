...
var W3C_OPTIONS = {
   ...
   filter: function(message) {
      if (
         /Element head is missing a required instance of child element title/.test(
            message))
         return false;
      if (/Attribute .+ not allowed on element .+ at this point/.test(message))
         return false;
      if (/Element .+ not allowed as child of element .+ in this context/.test(
            message))
         return false;
...
gulp.task('templates:app', function() {
   return gulp.src(source.templates.app.files)
      .pipe(changed(build.templates.app, {
         extension: '.html'
      }))
      .pipe(jade())
      .on("error", handleError)
...
gulp.task('templates:pages', function() {
   return gulp.src(source.templates.pages.files)
      .pipe(changed(build.templates.pages, {
         extension: '.html'
      }))
      .pipe(jade())
      .on("error", handleError)
...
gulp.task('templates:views', function() {
   return gulp.src(source.templates.views.files)
      .pipe(changed(build.templates.views, {
         extension: '.html'
      }))
      .pipe(jade())
      .on("error", handleError)
...
// default (no minify)
gulp.task('defaultprod', gulpsync.sync([
   'prod',
   'scripts:vendor',
   'scripts:app',
   'startprod'
...
// default (no minify)
gulp.task('defaulttest', gulpsync.sync([
   'test',
   'scripts:vendor',
   'scripts:app',
   'starttest'
...
gulp.task('EnvironmentConfig', function() {
   ...
   var git_branch, git_tag, git_last_commit_date;



   git_branch = git.branch();
   // => offline_data_process
   // git_tag = git.short(); // its git tag version
   git_tag = git.long(); // its git tag version
   // => 0.1.0
   // git_tag = 1.2;
   git_last_commit_date = git.date();

   var fs = require('fs');
   var fileName = '../server/environment.json';
   ...
   file.EnvironmentConfig.environment = 'Develpment Mode';
   file.EnvironmentConfig.serverUrl = 'http://localhost:8080/';
   ...
   file.EnvironmentConfig.version = git_tag;
   file.EnvironmentConfig.commit_date = git_last_commit_date;
   file.EnvironmentConfig.show_version_info = show_app_version;
   ...
   gulp.src(fileName)
      ...
      .pipe(gulp.dest('js/modules/directives'));
